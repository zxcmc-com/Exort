...
public class WirelessTerminalService {
    ...
    public WirelessTerminalService(ExortPlugin plugin, StorageKeys keys, CustomItems customItems, boolean enabled, int rangeChunks) {
        this.keys = keys;
    ...
    public ItemStack outputSample(ItemStack storedSample) {
        if (!isWireless(storedSample) || storedSample.getItemMeta() == null) return storedSample.clone();
        return extractFromStorage(storedSample.clone());
    }

    public boolean bind(Player player, ItemStack stack, String storageId, StorageTier tier, Location storageLoc) {
