...
public class SessionManager {
    ...
    private final com.zxcmc.exort.core.ExortPlugin plugin;
    private int wirelessTaskId = -1;
    private int renderTaskId = -1;

    public SessionManager(com.zxcmc.exort.core.ExortPlugin plugin) {
        this.plugin = plugin;
    }

    public com.zxcmc.exort.core.ExortPlugin getPlugin() {
        return plugin;
    }
    ...
    private void startWirelessWatcher() {
        com.zxcmc.exort.wireless.WirelessTerminalService wirelessService = plugin.getWirelessService();
        org.bukkit.Material storageCarrier = plugin.getStorageCarrier();
        if (wirelessService == null || storageCarrier == null) return;
        if (wirelessTaskId != -1) return;
        wirelessTaskId = Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, () -> {
            com.zxcmc.exort.wireless.WirelessTerminalService tickWireless = plugin.getWirelessService();
            org.bukkit.Material tickCarrier = plugin.getStorageCarrier();
            ...
            for (GuiSession session : snapshot) {
                ...
                org.bukkit.Location anchor = storageSession.getStorageLocation();
                if (!tickWireless.isEnabled()) {
                    plugin.getBossBarManager().showError(p, plugin.getLang().tr("message.wireless.disabled"), plugin.getStoragePeekTicks());
                    toClose.add(p.getUniqueId());
                    continue;
                }
                if (anchor == null || !anchor.isWorldLoaded()) {
                    plugin.getBossBarManager().showError(p, plugin.getLang().tr("message.wireless.missing_storage"), plugin.getStoragePeekTicks());
                    toClose.add(p.getUniqueId());
                    continue;
                }
                if (!tickWireless.inRange(anchor, p.getLocation())) {
                    plugin.getBossBarManager().showError(p, plugin.getLang().tr("message.wireless.out_of_range"), plugin.getStoragePeekTicks());
                    toClose.add(p.getUniqueId());
                    continue;
                }
    ...
    public boolean openSession(Player player, StorageCache cache, StorageTier tier, Block terminal, org.bukkit.Location storageLocation) {
        return openSessionInternal(player, cache, tier, terminal, storageLocation, false, false, false, SessionType.STORAGE, false);
    }

    public boolean openCraftingSession(Player player, StorageCache cache, StorageTier tier, Block terminal, org.bukkit.Location storageLocation) {
        return openSessionInternal(player, cache, tier, terminal, storageLocation, false, false, false, SessionType.CRAFTING, false);
    }

    public boolean openDebugSession(Player player, StorageCache cache, StorageTier tier, boolean write) {
        return openSessionInternal(player, cache, tier, null, null, !write, true, write, SessionType.STORAGE, false);
    }

    public boolean openWirelessSession(Player player, StorageCache cache, StorageTier tier, org.bukkit.Location storageLocation) {
        return openSessionInternal(player, cache, tier, null, storageLocation, false, false, false, SessionType.STORAGE, true);
    }

    private boolean openSessionInternal(Player player,
                                        StorageCache cache,
                                        StorageTier tier,
                                        Block terminal,
                                        ...
                                        SessionType type,
                                        boolean wireless) {
        LinkedHashSet<GuiSession> sessions = byStorage.computeIfAbsent(cache.getStorageId(), k -> new LinkedHashSet<>());
        ...
        GuiSession session = createSession(type, player, cache, tier, terminal, storageLocation, readOnly, wireless);
        ...
        if (storageLocation != null) {
            plugin.getDatabase().updatePlayerLastStorage(
                    player.getUniqueId(),
                    cache.getStorageId(),
    ...
    public boolean openSearch(Player player, SearchableSession parent) {
        ...
        if (!plugin.isDialogSupported()) {
            plugin.getBossBarManager().showError(player, plugin.getLang().tr("error.search.dialogs_unsupported"), 100);
            return false;
        }
    ...
    public void revalidateSessions() {
        for (GuiSession session : new ArrayList<>(byPlayer.values())) {
            ...
            var link = com.zxcmc.exort.core.network.TerminalLinkFinder.find(termBlock, plugin.getKeys(), plugin, plugin.getWireLimit(), plugin.getWireHardCap(), plugin.getWireMaterial(), plugin.getStorageCarrier());
            if (link.count() != 1 || link.data() == null || !session.getStorageId().equals(link.data().storageId())) {
                closeSearch(session.getViewer(), false);
                session.getViewer().closeInventory();
            }
        }
    ...
    private void scheduleRenderFlush() {
        ...
        renderTaskId = Bukkit.getScheduler().runTask(plugin, () -> {
            renderTaskId = -1;
            flushRenders();
        }).getTaskId();
    }
    ...
    private GuiSession createSession(SessionType type,
                                          Player player,
                                          StorageCache cache,
                                          StorageTier tier,
                                          Block terminal,
                                          ...
                                          boolean readOnly,
                                          boolean wireless) {
        long timeoutMs = Math.max(0L, plugin.getConfig().getLong("crafting.confirmTimeoutSeconds", 10) * 1000L);
        if (type == SessionType.CRAFTING) {
            CraftingState state = craftingStates.computeIfAbsent(cache.getStorageId(), k -> new CraftingState());
            return new CraftingSession(player, cache, tier, plugin.getLang(), plugin.getItemNameService(), terminal, storageLocation, this, readOnly, state, plugin.getCraftingRules(), titleFor(SessionType.CRAFTING), !plugin.isResourceMode(), timeoutMs, cache.getSortMode(), wireless);
        }
        return new StorageSession(player, cache, tier, plugin.getLang(), plugin.getItemNameService(), terminal, storageLocation, this, readOnly, titleFor(SessionType.STORAGE, wireless), !plugin.isResourceMode(), timeoutMs, cache.getSortMode(), wireless);
    }
    ...
    private net.kyori.adventure.text.Component titleFor(SessionType type, boolean wireless) {
        ...
        String prefixKey = type == SessionType.CRAFTING
                ? "resourceMode.craftingTerminal.gui.prefix"
                : "resourceMode.terminal.gui.prefix";
        String fontKey = type == SessionType.CRAFTING
                ? "resourceMode.craftingTerminal.gui.font"
                : "resourceMode.terminal.gui.font";
        String nameKey = type == SessionType.CRAFTING
                ? "item.crafting_terminal"
                : (wireless ? "item.wireless_terminal" : "item.terminal");
        String prefix = resourceMode
                ? plugin.getConfig().getString(prefixKey,
                type == SessionType.CRAFTING ? "§f૱" : "§fᾖ")
                : "";
        String font = resourceMode ? plugin.getConfig().getString(fontKey, "exort:default") : "minecraft:default";
        net.kyori.adventure.text.Component name = net.kyori.adventure.text.Component.text(plugin.getLang().tr(nameKey));
        if (prefix == null || prefix.isEmpty()) {
            return name;
        }
        net.kyori.adventure.text.Component prefixComponent = net.kyori.adventure.text.Component.text(prefix);
        try {
    ...
    private boolean isSessionValid(GuiSession session) {
        ...
        var link = com.zxcmc.exort.core.network.TerminalLinkFinder.find(termBlock, plugin.getKeys(), plugin, plugin.getWireLimit(), plugin.getWireHardCap(), plugin.getWireMaterial(), plugin.getStorageCarrier());
        return link.count() == 1 && link.data() != null && session.getStorageId().equals(link.data().storageId());
    }
