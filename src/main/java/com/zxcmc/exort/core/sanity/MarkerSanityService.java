...
public final class MarkerSanityService {
    ...
    public MarkerSanityService(ExortPlugin plugin,
    ...
    public void cleanupStaleMarkers(org.bukkit.Chunk chunk) {
        ...
        for (var key : keys) {
            ...
            if (raw.startsWith("bus_")) {
                ...
                if (!Carriers.matchesCarrier(block, busCarrier)) {
                    ...
                    com.zxcmc.exort.core.marker.BusMarker.get(plugin, block).ifPresent(data ->
                            plugin.getBusService().getOrCreateState(com.zxcmc.exort.bus.BusPos.of(block), data, block)
                    );
                }
                continue;
            }
            ...
            if (raw.startsWith("storage_")) {
                ...
                plugin.getDatabase().storageExists(storageId).thenAccept(exists -> {
                    if (exists) return;
