...
public final class DisplayCleanupService {
    ...
    public DisplayCleanupService(ExortPlugin plugin, Material wireCarrier, Material storageCarrier, Material terminalCarrier, Material monitorCarrier, Material busCarrier) {
        this.plugin = plugin;
    ...
    public void cleanupDisplays(org.bukkit.Chunk chunk) {
        ...
        for (var ent : chunk.getEntities()) {
            ...
            boolean trackedDisplay = tags.contains(DisplayTags.DISPLAY_TAG) || tags.contains(DisplayTags.HOLOGRAM_TAG);
            ...
            boolean keep =
                    ...
                    (Carriers.matchesCarrier(block, storageCarrier)
                            && (StorageMarker.get(plugin, block).isPresent() || StorageCoreMarker.isCore(plugin, block))) ||
                    (Carriers.matchesCarrier(block, terminalCarrier) && TerminalMarker.isTerminal(plugin, block)) ||
                    (Carriers.matchesCarrier(block, monitorCarrier) && MonitorMarker.isMonitor(plugin, block)) ||
                    (Carriers.matchesCarrier(block, busCarrier) && com.zxcmc.exort.core.marker.BusMarker.isBus(plugin, block));
            if (!keep) {
                display.remove();
            }
        }
    ...
    private void loadMonitorDisplayIds(org.bukkit.Chunk chunk, Set<UUID> monitorItems, Set<UUID> monitorTexts) {
    ...
    public void cleanupDisplayMarkers(org.bukkit.Chunk chunk) {
        var pdc = chunk.getPersistentDataContainer();
    ...
    private void cleanupDisplayMarker(org.bukkit.Chunk chunk, String rawKey, String type) {
        ...
        if (!(ent instanceof org.bukkit.entity.Display) || ent.isDead()) {
            DisplayMarker.clear(plugin, type, block);
        }
    }
}
