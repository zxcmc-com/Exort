...
public class Database implements AutoCloseable {
    ...
    public CompletableFuture<Void> ensureStorage(String id) {
        ...
        return CompletableFuture.runAsync(() -> {
            String sql = "INSERT OR IGNORE INTO storages(id, tier, sort_mode, created_at, updated_at) VALUES(?, NULL, ?, ?, ?)";
    ...
    public CompletableFuture<Void> setStorageTier(String storageId, String tierKey) {
        ...
        String defaultSort = defaultSortModeName();
        return CompletableFuture.runAsync(() -> {
            String sql = "INSERT INTO storages(id, tier, sort_mode, created_at, updated_at) " +
    ...
    public CompletableFuture<java.util.Optional<String>> getStorageSortMode(String storageId) {
        Objects.requireNonNull(storageId, "storageId");
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        return java.util.Optional.ofNullable(rs.getString("sort_mode"));
                    }
                }
            } catch (SQLException e) {
                plugin.getLogger().log(Level.SEVERE, "Failed to read storage sort mode for " + storageId, e);
            }
    ...
    public CompletableFuture<Void> setStorageSortMode(String storageId, String sortMode) {
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                plugin.getLogger().log(Level.SEVERE, "Failed to set storage sort mode for " + storageId, e);
            }
        }, executor);
    }

    public CompletableFuture<java.util.Optional<String>> getStorageTier(String storageId) {
        Objects.requireNonNull(storageId, "storageId");
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        return java.util.Optional.ofNullable(rs.getString("tier"));
                    }
                }
            } catch (SQLException e) {
    ...
    public CompletableFuture<Boolean> storageExists(String storageId) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                plugin.getLogger().log(Level.SEVERE, "Failed to check storage existence for " + storageId, e);
            }
            return false;
        }, executor);
    }

    public CompletableFuture<Void> updatePlayerLastStorage(java.util.UUID playerId,
                                                           String storageId,
                                                           String world,
                                                           int x,
                                                           int y,
        ...
        long now = Instant.now().getEpochSecond();
        return CompletableFuture.runAsync(() -> {
            String sql = "INSERT INTO players(player_uuid, last_storage_id, last_world, last_x, last_y, last_z, updated_at) " +
                    "VALUES(?, ?, ?, ?, ?, ?, ?) " +
                    "ON CONFLICT(player_uuid) DO UPDATE SET " +
                    "last_storage_id = excluded.last_storage_id, last_world = excluded.last_world, " +
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                plugin.getLogger().log(Level.SEVERE, "Failed to update last storage for " + playerId, e);
            }
        }, executor);
    }

    public CompletableFuture<java.util.Optional<PlayerLastStorage>> getPlayerLastStorage(java.util.UUID playerId) {
        Objects.requireNonNull(playerId, "playerId");
        return CompletableFuture.supplyAsync(() -> {
            String sql = """
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        return java.util.Optional.of(new PlayerLastStorage(
    ...
    public record PlayerLastStorage(String storageId, String tier, String world, int x, int y, int z, long updatedAt) {}

    public CompletableFuture<java.util.Optional<BusSettings>> loadBusSettings(BusPos pos, int slots) {
        Objects.requireNonNull(pos, "pos");
        return CompletableFuture.supplyAsync(() -> {
            String sql = "SELECT type, mode, filters FROM bus_settings WHERE world = ? AND x = ? AND y = ? AND z = ? LIMIT 1";
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        return java.util.Optional.of(new BusSettings(
                                pos,
                                com.zxcmc.exort.bus.BusType.fromString(type),
                                com.zxcmc.exort.bus.BusMode.fromString(mode),
                                decoded
                        ));
                    }
                }
    ...
    public CompletableFuture<Void> saveBusSettings(BusSettings settings, int slots) {
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                plugin.getLogger().log(Level.SEVERE, "Failed to save bus settings for " + settings.pos(), e);
            }
        }, executor);
    }
    ...
    public CompletableFuture<Void> writeSnapshot(String storageId, java.util.Collection<DbItem> items) {
        Objects.requireNonNull(storageId, "storageId");
        return CompletableFuture.runAsync(() -> {
            try {
                connection.setAutoCommit(false);
                try (PreparedStatement delete = connection.prepareStatement("DELETE FROM storage_items WHERE storage_id = ?")) {
                    delete.setString(1, storageId);
                    delete.executeUpdate();
                }
                String insertSql = "INSERT INTO storage_items(storage_id, item_key, item_blob, amount) VALUES(?, ?, ?, ?)";
                ...
                try (PreparedStatement update = connection.prepareStatement("UPDATE storages SET updated_at = ? WHERE id = ?")) {
    ...
    public CompletableFuture<Void> writeDelta(String storageId, java.util.Collection<DbItem> upserts, java.util.Collection<String> removals) {
        Objects.requireNonNull(storageId, "storageId");
        return CompletableFuture.runAsync(() -> {
            ...
            try {
                ...
                if (removals != null && !removals.isEmpty()) {
                    try (PreparedStatement delete = connection.prepareStatement("DELETE FROM storage_items WHERE storage_id = ? AND item_key = ?")) {
                        int batchSize = 0;
