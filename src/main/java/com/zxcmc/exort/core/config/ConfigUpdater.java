...
public final class ConfigUpdater {
    ...
    public static void update(JavaPlugin plugin, String resourceName) {
        ...
        if (existed) {
            try {
                ...
                plugin.getLogger().warning("Failed to load existing config, using defaults: " + e.getMessage());
            }
            try {
        ...
        String merged = mergeWithDefaults(defaultCfg, userCfg, userLines, resourceName, plugin, existed);
    ...
    private static String mergeWithDefaults(YamlConfiguration defaults, YamlConfiguration user, List<String> userLines, String resourceName, JavaPlugin plugin, boolean existed) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            String withoutComment = line.contains("#") ? line.substring(0, line.indexOf('#')).trim() : trimmed;
            ...
            if (isSection) {
                // Special handling for tiers: emit user section and skip defaults when user fully controls it
                if ("tiers".equalsIgnoreCase(fullPath) && existed) {
                    ...
                    if (rawTiersBody != null) {
                        ...
                        if (tiersObj instanceof org.bukkit.configuration.ConfigurationSection userTiers) {
                ...
                out.append(spaces(indent)).append(keyPart).append(": ").append(rendered).append(System.lineSeparator());
            }
        }
        ...
        if (existed) {
            ...
            if (!unknownLeaves.isEmpty()) {
                ...
                out.append("# Removed/unknown options preserved for reference:").append(System.lineSeparator());
                for (String key : unknownLeaves) {
                    out.append("# ").append(key).append(": ").append(scalarToString(user.get(key))).append(System.lineSeparator());
                }
            }
        }
        return out.toString();
    ...
    private static void renderSection(StringBuilder out, org.bukkit.configuration.ConfigurationSection section, int indent) {
        for (String key : section.getKeys(false)) {
            Object val = section.get(key);
            if (val instanceof org.bukkit.configuration.ConfigurationSection cs) {
                out.append(spaces(indent)).append(key).append(":").append(System.lineSeparator());
                renderSection(out, cs, indent + 2);
            } else {
                out.append(spaces(indent)).append(key).append(": ").append(scalarToString(val)).append(System.lineSeparator());
            }
        }
    }
    ...
    /**
     * Extracts the body of a top-level YAML section (excluding the header line).
     * Returned lines are raw (with original indentation).
     */
    ...
    /**
     * Normalizes indentation of a section body so that its minimum meaningful indent becomes targetIndent.
     */
    ...
    private static Set<String> collectUnknownLeaves(YamlConfiguration defaults, YamlConfiguration user) {
