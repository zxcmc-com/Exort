...
public class BossBarManager {
    ...
    public void showPeek(String storageId, StorageTier tier, Player player, long durationTicks) {
        ...
        storageManager.getOrLoad(storageId).thenAccept(cache -> Bukkit.getScheduler().runTask(plugin, () -> {
            ...
            String title = lang.tr("gui.bossbar", tier.displayName(), formatNumber(current), formatNumber(max), percent);
            double free = 1.0 - progress;
            showCustom(player, title, progress, freeColor(free), durationTicks, gen);
        }));
    }

    public void showWireStatus(int used, int max, boolean tooLong, boolean storageConnected, Player player, long durationTicks) {
        int gen = generations.compute(player.getUniqueId(), (id, val) -> val == null ? 1 : val + 1);
        cancelRemoval(player.getUniqueId());
        double progress = Math.min(1.0, Math.max(0.0, (double) used / Math.max(1, max)));
        String title = tooLong ? lang.tr("wire.too_long", used, max) : lang.tr("wire.status", used, max);
        if (storageConnected) {
    ...
    public void showMonitorItem(String storageId, String itemKey, String itemName, Player player, long durationTicks) {
        int gen = generations.compute(player.getUniqueId(), (id, val) -> val == null ? 1 : val + 1);
        cancelRemoval(player.getUniqueId());
        storageManager.getOrLoad(storageId).thenAccept(cache -> Bukkit.getScheduler().runTask(plugin, () -> {
            if (!player.isOnline()) return;
    ...
    private void showCustom(Player player, String title, double progress, BarColor color, long durationTicks, int gen) {
        BossBar bar = createOrGetBar(player);
        bar.setColor(color);
    ...
    private BossBar createOrGetBar(Player player) {
        return bars.compute(player.getUniqueId(), (id, existing) -> {
            if (existing != null) {
