...
public final class ItemModelUtil {
    ...
    public static ApplyResult applyItemModelDetailed(ItemMeta meta, String modelId) {
        ...
        // Prefer looking up methods on the Bukkit interface to avoid CraftBukkit reflection restrictions.
    ...
    private static ApplyResult invokePreferred(Object target, String methodName, Class<?> paramType, Object arg) {
        try {
            Method m;
            try {
                // Avoid meta.getClass() first (CraftMetaItem) - can be affected by Paper reflection rewriter.
            ...
            return ApplyResult.failure("method not found: " + methodName + "(" + paramType.getSimpleName() + ")");
        } catch (InvocationTargetException e) {
            Throwable cause = e.getCause() != null ? e.getCause() : e;
            return ApplyResult.failure(cause.getClass().getSimpleName() + ": " + (cause.getMessage() == null ? "" : cause.getMessage()));
        } catch (Throwable t) {
            return ApplyResult.failure(t.getClass().getSimpleName() + ": " + (t.getMessage() == null ? "" : t.getMessage()));
        }
    }
}
