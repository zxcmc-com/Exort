...
public class CustomItems {
    ...
    public CustomItems(StorageKeys keys,
                       Lang lang,
        ...
        this.craftingTerminalItemModel = craftingTerminalItemModel == null ? "" : craftingTerminalItemModel;
        ...
        this.wirelessDisabledItemModel = wirelessDisabledItemModel == null ? "" : wirelessDisabledItemModel;
    ...
    public ItemStack storageCoreItem() {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.itemName(Component.text(lang.tr("item.storage_core")).decoration(TextDecoration.ITALIC, false));
            meta.addItemFlags(ItemFlag.HIDE_ATTRIBUTES, ItemFlag.HIDE_ENCHANTS);
            PersistentDataContainer pdc = meta.getPersistentDataContainer();
    ...
    public ItemStack terminalItem() {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.itemName(Component.text(lang.tr("item.terminal")).decoration(TextDecoration.ITALIC, false));
            PersistentDataContainer pdc = meta.getPersistentDataContainer();
    ...
    public ItemStack craftingTerminalItem() {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.itemName(Component.text(lang.tr("item.crafting_terminal")).decoration(TextDecoration.ITALIC, false));
            PersistentDataContainer pdc = meta.getPersistentDataContainer();
    ...
    public ItemStack monitorItem() {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.itemName(Component.text(lang.tr("item.monitor")).decoration(TextDecoration.ITALIC, false));
    ...
    public ItemStack importBusItem() {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.itemName(Component.text(lang.tr("item.import_bus")).decoration(TextDecoration.ITALIC, false));
    ...
    public ItemStack exportBusItem() {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.itemName(Component.text(lang.tr("item.export_bus")).decoration(TextDecoration.ITALIC, false));
    ...
    public ItemStack wirelessTerminalItem(String owner, int charge) {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.itemName(Component.text(lang.tr("item.wireless_terminal")).decoration(TextDecoration.ITALIC, false));
            PersistentDataContainer pdc = meta.getPersistentDataContainer();
            pdc.set(keys.type(), PersistentDataType.STRING, "wireless_terminal");
            if (owner != null) {
                pdc.set(keys.wirelessOwner(), PersistentDataType.STRING, owner);
            }
            pdc.set(keys.wirelessCharge(), PersistentDataType.INTEGER, Math.max(0, Math.min(100, charge)));
    ...
    public boolean refreshItem(ItemStack stack, com.zxcmc.exort.wireless.WirelessTerminalService wirelessService, boolean inStorage) {
        if (stack == null || !stack.hasItemMeta()) return false;
    ...
    public void applyWirelessModel(ItemMeta meta, boolean charged, boolean linked, boolean enabled) {
        ...
        if (!enabled || !charged || !linked) {
            ItemModelUtil.applyItemModel(meta, wirelessDisabledItemModel.isEmpty() ? wirelessVanillaModel : wirelessDisabledItemModel);
        } else {
            ItemModelUtil.applyItemModel(meta, wirelessItemModel.isEmpty() ? wirelessVanillaModel : wirelessItemModel);
        }
    }
    ...
    public Optional<String> storageId(ItemStack stack) {
        ...
        return Optional.ofNullable(stack.getItemMeta().getPersistentDataContainer().get(keys.storageId(), PersistentDataType.STRING));
    }

    private void applyLore(ItemMeta meta, StorageTier tier, long currentAmount) {
        long max = tier.maxItems();
        double percent = Math.min(100.0, Math.max(0.0, (double) currentAmount / Math.max(1, max) * 100.0));
        String line = lang.tr("lore.storage.capacity", formatNumber(currentAmount), formatNumber(max), FORMAT_PERCENT.format(percent) + "%");
