...
public final class CacheDebugService {
    ...
    private void updateSummaryTask() {
        ...
        if (shouldRun && summaryTaskId == -1) {
            summaryTaskId = Bukkit.getScheduler().runTaskTimer(plugin, this::flushSummary, SUMMARY_INTERVAL_TICKS, SUMMARY_INTERVAL_TICKS).getTaskId();
    ...
    private Component formatSummary(Summary snapshot) {
        ...
        if (snapshot.lastEvictStorageId != null) {
            base = base.append(Component.text(" lastEvict=", NamedTextColor.GRAY))
    ...
    private static final class Summary {
        ...
        private boolean isEmpty() {
            return loads == 0 && flushes == 0 && evicts == 0 && addOps == 0 && removeOps == 0 && removeAllFail == 0;
        }
