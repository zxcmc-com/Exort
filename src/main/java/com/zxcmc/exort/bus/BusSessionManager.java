...
public class BusSessionManager {
    ...
    public boolean openSession(Player player, Block busBlock) {
        ...
        BusSession session = new BusSession(player, this, state, lang, plugin.getBossBarManager(), !plugin.isResourceMode(), title, busBlock);
    ...
    public BusLinkStatus resolveStatus(BusState state) {
        ...
        var link = TerminalLinkFinder.find(busBlock,
                plugin.getKeys(),
                plugin,
                plugin.getWireLimit(),
    ...
    public record BusLinkStatus(StorageState storageState, String storageId, String storageName, String inventoryName, boolean loopDisabled) {
    }
}
