...
public final class BusRegistry {
    ...
    public void saveSettings(BusState state) {
        ...
        BusSettings settings = new BusSettings(state.pos(), state.type(), state.mode(), state.filters());
    ...
    public void scanChunk(Chunk chunk) {
        Set<org.bukkit.NamespacedKey> keys = chunk.getPersistentDataContainer().getKeys();
        ...
        for (var key : keys) {
            ...
            BusMarker.get(plugin, block).ifPresent(data -> getOrCreateState(BusPos.of(block), data, block));
        }
    }
    ...
    private void loadSettingsIfMissing(BusState state) {
        ...
        database.loadBusSettings(pos, FILTER_SLOTS).thenAccept(opt -> {
