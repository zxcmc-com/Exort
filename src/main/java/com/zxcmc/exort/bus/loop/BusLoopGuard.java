...
public final class BusLoopGuard {
    ...
    public static boolean filtersIntersect(BusMode modeA, Set<String> keysA, BusType typeA, boolean importSideSensitiveA,
                                           BusMode modeB, Set<String> keysB, BusType typeB, boolean importSideSensitiveB) {
        if (modeA == BusMode.DISABLED || modeB == BusMode.DISABLED) return false;
        // If inventory is side-sensitive, import ALL + export ALL cannot loop by design (slot separation).
        if (modeA == BusMode.ALL && typeA == BusType.IMPORT && importSideSensitiveA
                && modeB == BusMode.ALL && typeB == BusType.EXPORT) {
            return false;
        }
        if (modeB == BusMode.ALL && typeB == BusType.IMPORT && importSideSensitiveB
                && modeA == BusMode.ALL && typeA == BusType.EXPORT) {
            return false;
        }
        BusMode a = modeA;
        BusMode b = modeB;
