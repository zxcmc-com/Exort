...
public class StorageDisplayManager extends BaseCarrierDisplayManager {
    public StorageDisplayManager(Plugin plugin,
                                 ...
                                 double offsetY,
                                 double offsetZ) {
        super(plugin, carrierMaterial, displayModelId, displayBaseMaterial, displayScale, offsetX, offsetY, offsetZ, "storage");
    }
    ...
    protected void decorateMeta(ItemMeta meta, Block block) {
        StorageMarker.get(plugin, block).ifPresentOrElse(data ->
                        meta.displayName(Component.text(data.tier().displayName()).decoration(TextDecoration.ITALIC, false)),
                () -> {
                    if (StorageCoreMarker.isCore(plugin, block)) {
                        meta.displayName(Component.text(coreName()).decoration(TextDecoration.ITALIC, false));
                    }
                });
    }
    ...
    private String coreName() {
        if (plugin instanceof com.zxcmc.exort.core.ExortPlugin exort) {
