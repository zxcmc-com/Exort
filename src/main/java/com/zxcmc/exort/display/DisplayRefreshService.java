...
public final class DisplayRefreshService {
    private static final org.bukkit.block.BlockFace[] FACES = new org.bukkit.block.BlockFace[]{
    ...
    public DisplayRefreshService(ExortPlugin plugin,
    ...
    public void refreshNetworkFrom(Block block) {
        ...
        if (hardCap == 0) return;
        boolean isWire = Carriers.matchesCarrier(block, wireMaterial) && WireMarker.isWire(plugin, block);
        if (isWire) {
    ...
    private void refreshFromWire(Block start, int hardCap, org.bukkit.Material wireMaterial) {
        ...
        while (!queue.isEmpty() && visited.size() <= hardCap) {
            ...
            for (var face : FACES) {
                ...
                if (Carriers.matchesCarrier(next, plugin.getTerminalCarrier()) && TerminalMarker.isTerminal(plugin, next)) {
                    terminals.add(next);
                } else if (Carriers.matchesCarrier(next, plugin.getMonitorCarrier()) && MonitorMarker.isMonitor(plugin, next)) {
                    monitors.add(next);
                } else if (Carriers.matchesCarrier(next, plugin.getBusCarrier()) && BusMarker.isBus(plugin, next)) {
                    buses.add(next);
                } else if (Carriers.matchesCarrier(next, plugin.getStorageCarrier()) && StorageMarker.get(plugin, next).isPresent()) {
