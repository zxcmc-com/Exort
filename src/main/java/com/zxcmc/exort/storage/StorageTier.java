...
public class StorageTier {
    ...
    public static void loadFromConfig(ConfigurationSection section, Logger logger) {
        ...
        for (String key : section.getKeys(false)) {
            ...
            if (matRaw == null || matRaw.isBlank()) {
                ...
                if (mat == null) {
                    logger.warning("Tier '" + key + "': unknown material '" + matRaw + "', using fallback " + fallback);
                    mat = fallback;
                }
            }
    ...
    private static long parseMaxItems(Object raw, Logger logger, String tierKey) {
        ...
        if (raw instanceof Number num) {
            ...
            if (value <= 0) {
                logger.warning("Tier '" + tierKey + "': maxItems must be positive, using default " + defaultValue);
                return defaultValue;
            }
        ...
        try {
            ...
            logger.warning("Tier '" + tierKey + "': invalid maxItems '" + str + "', using default " + defaultValue);
            return defaultValue;
        }
    }
